package com.vat.icare.chats.holder;
...
public class MessageHolder extends RecyclerView.ViewHolder {
    ...
    public void setRightMessage(String userid, final String message, long time, String type) {
        ...
        final LinearLayout messageLayoutLeft = view.findViewById(R.id.message_relative_left);

        final LinearLayout messageLayoutRight = view.findViewById(R.id.message_relative_right);
        final TextView messageTextRight = view.findViewById(R.id.txtv_received_message);
            ...
            public void onDataChange(final DataSnapshot dataSnapshot) {
                try {
                    ...
                    if (!image.equals("default")) {
                        ...
                        messageImageRight.setImageBitmap(decodedByte);

                    } else {
                        messageImageRight.setImageResource(R.drawable.logo);
                    }
                } catch (Exception e) {
            ...
            public void onDataChange(final DataSnapshot dataSnapshot) {
                try {
                    ...
                    if (!image.equals("default")) {
                        ...
                        byte[] decodedString = Base64.decode(image, Base64.DEFAULT);
                        Bitmap decodedByte = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length);
                        messageImageLeft.setImageBitmap(decodedByte);

                    } else {
                        messageImageLeft.setImageResource(R.drawable.logo);
                    }
                } catch (Exception e) {
                    Log.d(TAG, "userDatabase exception: " + e.getMessage());
                    e.printStackTrace();
